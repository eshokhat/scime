...
import { Activity, Globe, Network, BookOpen, TrendingUp, Loader2, Microscope, ShieldCheck } from 'lucide-react';

export default function App() {
  // Navigation state
  const [activeTab, setActiveTab] = useState('h1');
  
  // Selection state (values must match Python backend expectations)
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      alert("Failed to fetch data. Please ensure the Python backend (api.py) is running on port 8000 without errors.");
    } finally {
      setIsLoading(false);
    }
  };

  // Auto-fetch data whenever target or compare country changes
  useEffect(() => {
    fetchData();
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 min-h-[500px] relative">
            ...
            <div className="animate-in fade-in duration-500">
              <div className="mb-6 flex justify-between items-start">
                <div>
                  ...
                  <p className="text-slate-500 mt-1">Impact of political events on the growth of joint scientific research.</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
                ...
                {/* GLOBAL BROKERS WIDGET */}
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-5 overflow-y-auto max-h-[400px]">
                  ...
                  {globalBrokers.length === 0 ? (
                    <p className="text-sm text-slate-500">No external brokers found for this dyad.</p>
                  ) : (
                      ...
                      {globalBrokers.map((broker, idx) => (
                        <div key={idx}>
                          <div className="flex justify-between text-sm mb-1">
                            ...
                            <span className="text-slate-500">{broker.papers} papers</span>
                          </div>
                  ...
                  <div className="mt-6 pt-4 border-t border-slate-200">
                    <p className="text-xs text-slate-500">Western institutions act as an infrastructure bridge (Western Umbrella effect).</p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-white border border-slate-200 p-4 rounded-xl flex items-center">
                  <div className="bg-slate-100 p-3 rounded-lg mr-4"><Activity className="w-5 h-5 text-slate-600"/></div>
                  <div>
                    <p className="text-sm text-slate-500">Average per year (2015-2019)</p>
                    <p className="text-xl font-bold text-slate-800">{summary.pre} papers</p>
                  </div>
                </div>
                <div className="bg-white border border-slate-200 p-4 rounded-xl flex items-center">
                  <div className="bg-slate-100 p-3 rounded-lg mr-4"><TrendingUp className="w-5 h-5 text-slate-600"/></div>
                  <div>
                    <p className="text-sm text-slate-500">Average per year (2021-2025)</p>
                    <p className="text-xl font-bold text-slate-800">{summary.post} papers</p>
                  </div>
                </div>
                <div className={`${summary.growth >= 0 ? 'bg-emerald-50 border-emerald-100' : 'bg-red-50 border-red-100'} p-4 rounded-xl flex items-center`}>
                  <div className={`${summary.growth >= 0 ? 'bg-emerald-100' : 'bg-red-100'} p-3 rounded-lg mr-4`}>
                    <TrendingUp className={`w-5 h-5 ${summary.growth >= 0 ? 'text-emerald-600' : 'text-red-600'}`}/>
                  </div>
                  <div>
                    <p className={`text-sm ${summary.growth >= 0 ? 'text-emerald-700' : 'text-red-700'}`}>Post-Accords Growth</p>
                    <p className={`text-xl font-bold ${summary.growth >= 0 ? 'text-emerald-800' : 'text-red-800'}`}>
          ...
          {activeTab === 'h4' && (
            <div className="animate-in fade-in duration-500">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-8">
                ...
                <div className="col-span-1 bg-slate-50 border border-slate-200 rounded-xl p-6 flex flex-col justify-center items-center text-center">
                  ...
                  <p className="text-sm text-slate-500">
                    Proportion of joint research conducted in politically agnostic STEM fields versus Humanities and Social Sciences.
                  </p>
                </div>
                ...
                <div className="col-span-1 md:col-span-2">
                  <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider mb-6 flex items-center">
                    ...
                    Top 5 Collaborative Subject Areas
                  </h3>
              ...
              {/* TEMPORAL DYNAMICS TIMELINE (NEW) */}
              <div className="mt-8 bg-white border border-slate-200 rounded-xl p-6">
                <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider mb-6 flex items-center">
                  <Activity className="w-4 h-4 mr-2 text-indigo-600" />
                  Temporal Dynamics of Subject Areas
                </h3>
